...
function renderApps(){const q=($("appSearch").value||"").toLowerCase();$("appList").innerHTML=apps.filter(a=>a.name.toLowerCase().includes(q)).map((a,i)=>`<div class="appItem"><span class="appIcon">📱</span><b style="flex:1">${esc(a.name)}</b><button onclick="openApp('${esc(a.package||"")}')">Open</button></div>`).join("")||"<p class='hint'>Refresh in the Android APK to detect installed apps.</p>"}
function refreshApps(){if(window.AndroidBridge&&AndroidBridge.getApps){try{apps=JSON.parse(AndroidBridge.getApps());saveAll()}catch{}}renderApps()}
function openApp(pkg){if(window.AndroidBridge&&AndroidBridge.openApp)AndroidBridge.openApp(pkg);else alert("App launching is available in the Android APK.")}
